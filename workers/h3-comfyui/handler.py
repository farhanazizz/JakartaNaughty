...
def handler(job):
    ...
    try:
        ...
        def _append_media(filename, subfolder, media_type, item_meta=None):
            """Fetch file from Comfy /view and append to output_data (images or VHS gifs/mp4)."""
            if media_type == "temp":
                print(
                ...
                return
            if not filename:
                warn_msg = f"Skipping media in node due to missing filename: {item_meta}"
            ...
            if os.environ.get("BUCKET_ENDPOINT_URL"):
                    ...
                    output_data.append(
                        {
                            ...
                            "type": "s3_url",
                            "data": s3_url,
                            "format": (item_meta or {}).get("format"),
                        }
                    )
                except Exception as e:
                ...
                try:
                    ...
                    print(f"worker-comfyui - Encoded {filename} as base64 ({len(file_bytes)} bytes)")
                except Exception as e:
        ...
        for node_id, node_output in outputs.items():
            # Standard SaveImage / PreviewImage
            ...
            # VHS_VideoCombine / video nodes put MP4 under "gifs" (and sometimes "videos")
            for video_key in ("gifs", "videos"):
                if video_key not in node_output:
                    continue
                items = node_output[video_key] or []
                print(
                    f"worker-comfyui - Node {node_id} contains {len(items)} {video_key} item(s) (VHS/video)"
                )
    ...
    # Expose videos alias for H3/VHS consumers (same payloads as images entries that are mp4)
    videos = [
        item
        for item in output_data
        if str(item.get("filename", "")).lower().endswith((".mp4", ".webm", ".mkv"))
        or str(item.get("format", "")).startswith("video/")
    ]
    if videos:
    ...
    if not output_data and errors:
        ...
        print(
            f"worker-comfyui - Job completed successfully, but no images/gifs/videos were collected from history."
        )
        final_result["status"] = "success_no_images"
        final_result["images"] = []

    print(f"worker-comfyui - Job completed. Returning {len(output_data)} media file(s).")
    return final_result
